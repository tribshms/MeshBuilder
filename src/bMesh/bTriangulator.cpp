...
/***************************************************************************
**
** bTriangulator.cpp  A triangulation routine based on Tipper's convex hull 
...
static
double vecprod(int p0,int p1,int p2, bNode** p)
{
  return
    (p[p1]->getX() - p[p0]->getX()) * 
    (p[p2]->getY() - p[p0]->getY()) -
...
#if defined(DEBUG_PRINT)
void edge::print(const point p[]) const
{
   p[from].print();
   p[to].print();
}
...
bool edge::visible(bNode* p[],int i) const 
{
...
static
bool needswap(int i1, int i2, int i3, int i4, bNode* p[])
{
  ...
  double p1X = p[i3]->getX() - p[i2]->getX();
  double p1Y = p[i3]->getY() - p[i2]->getY();

  double p2X = p[i4]->getX() - p[i2]->getX();
  double p2Y = p[i4]->getY() - p[i2]->getY();

  double p3X = p[i4]->getX() - p[i1]->getX();
  double p3Y = p[i4]->getY() - p[i1]->getY();

  double p4X = p[i3]->getX() - p[i1]->getX();
  double p4Y = p[i3]->getY() - p[i1]->getY();

  const double cost1 = p1X * p2X + p1Y * p2Y;
  const double cost2 = p3X * p4X + p3Y * p4Y;
...
static
int tt_swap(int tint, edge e[], bNode* p[])
{
  int& to = e[tint].to;
...
void start_aligned_point(int &lower_hull_pos, 
                         int &upper_hull_pos, 
                         int &next_edge,
			 int &next_point,
			 int npoints, bNode** p,  edge *edges,
			 cyclist &hull)
{
...
void triangulate(int npoints, bNode* p[], int *pnedges, edge** edges_ret){
  ...
  for (int i=next_point;i<npoints;i++){
    saved_edge=-1;
      if (fabs(p[i-1]->getX() - p[i]->getX()) < 0.000001 &&
          fabs(p[i-1]->getY() - p[i]->getY()) < 0.000001) {
      cout << "*****triangulate: skipping duplicate point " << i << endl;
      cout << "p[i-1] = " << p[i-1]->getID() << " " << p[i-1]->getX() << " " << p[i-1]->getY() << endl;
      cout << "p[i] = " << p[i]->getID() << " " << p[i]->getX() << " " << p[i]->getY() << endl;
...
static
void tt_verify_sort(int npoints, bNode** p)
{
  for(int ipoint=1; ipoint < npoints; ++ipoint){
    assert(p[ipoint-1]->getX() <= p[ipoint]->getX());
    if (p[ipoint-1]->getX() == p[ipoint]->getX())
      assert(p[ipoint-1]->getY() <= p[ipoint]->getY());
  }
}

void tt_sort_triangulate(int npoints, bNode** p,
  ...
  { 
	...
	if (edges[iedge].lef == -1) {
	  ...
	  mark_as_visited(edges[iedge].let,iedge,edges, edges_visit, ielem_current);
          if (ielem >= nelem) {
              cout << "ilelem >= nelem" << " iedge = " << iedge << endl;
          }
    ...
    cout << "ielem = " << ielem << " nelem = " << nelem << endl;
    assert(ielem == nelem);
  }
...
#define SWAP_E(EI,EJ) \
	...
	cout << "These points are aligned: "
	     << elems[ielem].p1 << "(" << p[elems[ielem].p1]->getX()
	     << "," << p[elems[ielem].p1]->getY() << "), " 
	     << elems[ielem].p2 << "(" << p[elems[ielem].p2]->getX()
	     << "," << p[elems[ielem].p2]->getY() << "), " 
	     << elems[ielem].p3 << "(" << p[elems[ielem].p3]->getX()
	     << "," << p[elems[ielem].p3]->getY() << ")" 
	     << endl;
      }
      assert(v != 0.);
      if (v>0) {
