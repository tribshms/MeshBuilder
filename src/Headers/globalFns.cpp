...
**  
**  Uses bNode objects in NewTriCCW, InNewTri, Intersect
**
***************************************************************************/
...
/***************************************************************************
**  
**  globalFns::UnitVector( bEdge* ePtr )
** 
***************************************************************************/
std::vector< double > UnitVector( bEdge* ePtr )
{
   assert( ePtr != 0 );
   std::vector<double> oxy(2), dxy(2);
   oxy = ePtr->getOriginPtr()->get2DCoords();
   dxy = ePtr->getDestinationPtr()->get2DCoords();
   double dx = dxy[0] - oxy[0];
   double dy = dxy[1] - oxy[1];
   double mag = sqrt( dx * dx + dy * dy );
   std::vector< double > uvect(2);
   uvect[0] = dx / mag;
   uvect[1] = dy / mag;
   return uvect;
}
...
** 
***************************************************************************/
double FindCosineAngle0_2_1( std::vector< double > &p0,
...
** 
***************************************************************************/
int TriPasses( std::vector< double > &ptest,
...
** 
***************************************************************************/
int PointsCCW( std::vector<double>& p0, 
               std::vector<double>& p1, 
               std::vector<double>& p2 )
{
   assert( &p0 != 0 && &p1 != 0 && &p1 != 0 );
...
** 
***************************************************************************/
int InNewTri( std::vector< double > &xy, bTriangle *ct )
{
   int j;
   bNode *vtx;
   std::vector<double> xy1(2), xy2(2);
   for( j=0; j<3; j++ ){
      vtx = (bNode *) ct->pPtr(j);
...
/***************************************************************************
**  
**  globalFns::Intersect( bEdge * ae, bEdge * be )
** 
**  Edge Intersection routine. Meandering turned off.
**
***************************************************************************/
int Intersect( bEdge * ae, bEdge * be )
{
   bNode * lnode;
   ...
   lnode = (bNode *) ae->getDestinationPtrNC();
   ...
   lnode = (bNode *) be->getDestinationPtrNC();
...
**
***************************************************************************/
bEdge* IntersectsAnyEdgeInList( bEdge* edge, std::list<bEdge*>& edglistRef )
{
   bEdge * ce;
   std::list<bEdge*>::iterator edgIter;
   if( !edge ){
   ...
   for (edgIter = edglistRef.begin();
        edgIter != edglistRef.end(); edgIter++) {
      ce = (*edgIter);
      if( edge != ce ){
         if( Intersect( edge, ce ) ) return( ce );
      }     
   }
   assert( edgIter == edglistRef.end() );
   return( NULL );
}
...
**
***************************************************************************/
double InterpSquareGrid( double xgen, double ygen, 
                         double** elev, int nx, int ny,
                         int nodata )
{
   int nodatacount = 0;
