...
/*************************************************************************/
/*! This function finds the connected workflow induced by the
    partitioning vector.
    ...
    \param cptr is the ptr structure of the CSR representation of the 
           workflow. The length of this vector must be graph->nvtxs+1.
    \param cind is the indices structure of the CSR representation of 
           the workflow. The length of this vector must be graph->nvtxs.

    \returns the number of workflow that it found.

    \note The cptr and cind parameters can be NULL, in which case only the
          number of connected workflow is returned.
*/
/*************************************************************************/
...
idx_t IsConnected(graph_t *graph, idx_t report)
{
  ...
  if (ncmps != 1 && report)
    printf("The graph is not connected. It has %"PRIDX" connected workflow.\n", ncmps);
...
idx_t IsConnectedSubdomain(ctrl_t *ctrl, graph_t *graph, idx_t pid, idx_t report)
{
  ...
  if (ncmps > 1 && report) {
    printf("The graph has %"PRIDX" connected workflow in partition %"PRIDX":\t", ncmps, pid);
    for (i=0; i<ncmps; i++) {
      wgt = 0;
...
/*************************************************************************/
/*! This function identifies the number of connected workflow in a graph
...
/*************************************************************************/
/*! This function finds all the connected workflow induced by the
...
void EliminateComponents(ctrl_t *ctrl, graph_t *graph)
{
  ...
  IFSET(ctrl->dbglvl, METIS_DBG_CONTIGINFO, 
      printf("I found %"PRIDX" workflow, for this %"PRIDX"-way partition\n",
          ncmps, nparts)); 

  /* There are more workflow than partitions */
  if (ncmps > nparts) {
    ...
    /* Get a CSR representation of the workflow-2-partitions mapping */
    for (i=0; i<ncmps; i++) 
