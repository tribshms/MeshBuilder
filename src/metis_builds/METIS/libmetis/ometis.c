...
/*! This routine is similar to its non 'CC' counterpart. The difference is
    that after each tri-section, the connected workflow of the original
         ...
         idx_t lastvtx)
{
  ...
  if (ctrl->dbglvl&METIS_DBG_INFO) {
    if (ncmps > 2)
      printf("  Bisection resulted in %"PRIDX" connected workflow\n", ncmps);
  }
...
/*! This function takes a graph and generates a set of graphs, each of 
    ...
    \param ncmps is the number of connected workflow.
    \param cptr is an array of size ncmps+1 that marks the start and end
           ...
           workflow (i.e., cptr, cind is in CSR format).
