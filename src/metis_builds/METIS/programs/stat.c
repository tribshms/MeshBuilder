  ...
  if (ncmps == nparts)
    ...
    if (IsConnected(graph, 0)) {
      ...
      printf(" - There are %"PRIDX" non-contiguous partitions.\n"
             "   Total workflow after removing the cut edges: %"PRIDX",\n"
             "   max workflow: %"PRIDX" for pid: %"PRIDX".\n",
          nover, ncmps, imax(nparts, cpwgts,1), (idx_t)iargmax(nparts, cpwgts,1));
    }
    else {
      printf(" - The original graph had %"PRIDX" connected workflow and the resulting\n"
             "   partitioning after removing the cut edges has %"PRIDX" workflow.",
